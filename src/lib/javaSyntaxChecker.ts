...
class JavaSyntaxChecker {
    validateSyntax(code: string): SyntaxValidationResult {
        ...
        // Check for specific Roboter class requirement
        if (!code.includes('class Roboter') && !code.includes('public class Roboter')) {
            errors.push({
                line: 1,
                column: 1,
                message: "Missing Roboter class",
                explanation: "This challenge requires a class named 'Roboter'. Use 'public class Roboter { ... }'"
            });
        }
        ...
        if (!code.includes('public static void main') && !code.includes('static void main')) {
            errors.push({
                ...
                explanation: "Java programs need a main method to run. Add 'public static void main(String[] args) { ... }'"
            });
        }
        ...
        if (braceCount !== 0) {
            errors.push({
                line: 1,
                column: 1,
                message: `Unbalanced braces (${Math.abs(braceCount)} ${braceCount > 0 ? 'opening' : 'closing'} brace${Math.abs(braceCount) > 1 ? 's' : ''} missing)`,
                explanation: "Check that all opening braces '{' have corresponding closing braces '}'."
            });
        }
        ...
        if (parenCount !== 0) {
            errors.push({
                line: 1,
                column: 1,
                message: `Unbalanced parentheses (${Math.abs(parenCount)} ${parenCount > 0 ? 'opening' : 'closing'} parenthesis${Math.abs(parenCount) > 1 ? 'es' : ''} missing)`,
                explanation: "Check that all opening parentheses '(' have corresponding closing parentheses ')'."
            });
        }
        ...
        if (bracketCount !== 0) {
            errors.push({
                line: 1,
                column: 1,
                message: `Unbalanced brackets (${Math.abs(bracketCount)} ${bracketCount > 0 ? 'opening' : 'closing'} bracket${Math.abs(bracketCount) > 1 ? 's' : ''} missing)`,
                explanation: "Check that all opening brackets '[' have corresponding closing brackets ']'."
            });
        }
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (trimmedLine && 
                ...
                if (trimmedLine.includes('System.out.print') || 
                    ...
                    trimmedLine.includes('++') ||
                    trimmedLine.includes('--') ||
                    (trimmedLine.includes('(') && trimmedLine.includes(')') && !trimmedLine.includes('{')) // Method calls
                ) {
                    errors.push({
                        line: lineNumber,
                        column: line.length,
                        message: "Missing semicolon",
                        explanation: "This statement appears to be missing a semicolon ';' at the end."
                    });
                }
            }
            ...
            if (line.includes('System.out.println') && !line.includes('(')) {
                errors.push({
                    ...
                    explanation: "System.out.println requires parentheses around the text to print."
                });
            }

            // Check for incorrect main method signature
            if (line.includes('public static void main') && !line.includes('String[] args')) {
                errors.push({
                    line: lineNumber,
                    column: line.indexOf('public static void main'),
                    message: "Incorrect main method signature",
                    explanation: "The main method must have the parameter 'String[] args'."
                });
            }
            ...
            if (line.includes('public class') && !line.includes('{')) {
                errors.push({
                    ...
                    explanation: "Class declarations must be followed by an opening brace '{'."
                });
            }
            ...
            if (line.includes('public static void main') && !line.includes('{')) {
                errors.push({
                    ...
                    explanation: "Method declarations must be followed by an opening brace '{'."
                });
            }
                ...
                if (cleanWord) {
                    ...
                    if (misspellings[cleanWord.toLowerCase()]) {
                        errors.push({
                            ...
                            explanation: `Did you mean '${misspellings[cleanWord.toLowerCase()]}'? Check the spelling of Java keywords.`
                        });
                    }
                }
            });
